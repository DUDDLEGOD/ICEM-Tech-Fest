...
import { Trophy, Users, X, Zap, Calendar, Target, Shield, Clock, Hash, MapPin, ChevronRight } from 'lucide-react';
...
export const EventCatalog: React.FC<{ onRegister: (id: string) => void }> = ({ onRegister }) => {
      ...
      >
        {EVENTS.map((event) => (
          <motion.div 
            ...
            whileHover="hover"
            className="group relative h-[440px] cursor-none"
          >
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2.5 px-3 py-1 bg-amber-500/5 border border-amber-500/20 rounded-full w-fit">
                        <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">{event.department.split(' ')[0]}</span>
                      </div>
      ...
      <AnimatePresence>
        {selectedEvent && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-black/95 backdrop-blur-3xl overflow-y-auto cursor-none">
            ...
            >
              <button onClick={() => setSelectedEvent(null)} className="absolute top-8 right-8 p-3 text-slate-500 hover:text-white hover:bg-white/5 rounded-full transition-all z-20"><X size={24}/></button>
              <div className="relative z-10 p-10 md:p-16 space-y-12 text-left">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-10 border-b border-white/5 pb-12">
                  <div className="space-y-5 max-w-3xl">
                    <div className="flex items-center gap-4">
                      <div className="px-4 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full text-amber-500 text-[11px] font-black uppercase tracking-widest flex items-center gap-2 leading-none"><Target size={14}/> {selectedEvent.department}</div>
                    </div>
                    <h2 className="text-5xl md:text-7xl font-futuristic font-black uppercase text-white tracking-tighter italic leading-none">{selectedEvent.name}</h2>
                    <p className="text-xl text-slate-400 italic font-medium leading-relaxed">"{selectedEvent.tagline}"</p>
                  </div>
                  <div className="flex-shrink-0">
                    <div className="bg-amber-500 border border-amber-400 px-8 py-6 rounded-3xl text-center shadow-2xl shadow-amber-500/10">
                      ...
                      <p className="text-3xl font-futuristic font-black text-black leading-none">{selectedEvent.prizePool}</p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="relative space-y-8 pl-10 border-l border-white/10 ml-2">
                      {selectedEvent.rounds.map((round, i) => (
                        <div key={i} className="relative">
                          <div className="absolute -left-[51px] top-0 w-6 h-6 rounded-full bg-stone-900 border-2 border-amber-500/50 flex items-center justify-center text-[10px] font-black text-amber-500 z-10">{i+1}</div>
                ...
                <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-10">
                  <div className="flex flex-wrap justify-center md:justify-start gap-10">
                    <div className="flex items-center gap-3"><Calendar size={16} className="text-amber-500"/><span className="text-[11px] font-black text-white uppercase tracking-widest">March 14, 2026</span></div>
                    <div className="flex items-center gap-3"><Users size={16} className="text-amber-500"/><span className="text-[11px] font-black text-white uppercase tracking-widest">{selectedEvent.minTeam}-{selectedEvent.maxTeam} Combatants</span></div>
                  </div>
                  <button 
