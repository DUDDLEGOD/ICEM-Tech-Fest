      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 left-0 h-full w-[280px] md:w-[350px] glass border-r border-cyan-400/20 z-[1000] flex flex-col"
          >
            {/* Header */}
            <div className="p-8 border-b border-white/5 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-[#06b6d4] to-purple-400 rounded-xl flex items-center justify-center text-slate-950 font-black italic shadow-lg shadow-[#06b6d4]/15">T</div>
                <div>
                  <h4 className="font-futuristic text-xs font-black uppercase tracking-widest text-white">Menu</h4>
                  <span className="text-[8px] font-bold text-cyan-300/50 uppercase tracking-[0.3em]">System Navigation</span>
                </div>
              </div>
              <button
                onClick={onClose}
                ...
                <div className="space-y-1">
                  {menuItems.map((item, i) => (
                    <button
                      ...
                      className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl hover:bg-cyan-400/10 text-slate-400 hover:text-cyan-300 transition-all group"
                    >
                ...
                <div className="grid grid-cols-1 gap-2">
                  {deptItems.map((dept, i) => (
                    <button
                      ...
                      className="flex items-center gap-4 px-4 py-3 rounded-xl border border-white/5 hover:border-purple-400/30 bg-white/[0.02] hover:bg-purple-400/5 transition-all group"
                    >
                      <div className="w-8 h-8 rounded-lg bg-purple-400/10 flex items-center justify-center text-purple-300">
                        <dept.icon size={14} />
                      </div>
